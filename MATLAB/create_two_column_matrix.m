...
crispian_matrix = [C_Labels, DataVec];
half_bandwidth = 20;
for i = 1:length(crispian_matrix)
    label = crispian_matrix(i,1);
    if label ~= 0 && crispian_matrix(i-1,1) == 0
        crispian_matrix(i-half_bandwidth:i,1) = label;
        crispian_matrix(i:i+half_bandwidth,1) = label;
        i = i + half_bandwidth + 1;
    end
...
writematrix(crispian_matrix, 'crispian_matrix.csv')